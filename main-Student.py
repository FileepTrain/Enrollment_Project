...
def add_department(db):
    ...
    while not unique_name or not unique_abbreviation or not unique_chair_name or not unique_building_office:
        ...
        try:
            ...
            print("Office Number should be an integer. Try again.")
            continue
        ...
        office_count: int = collection.count_documents({"building": building, "office": office})
        unique_building_office = office_count == 0
        if not unique_name:
            print("We already have a department by that name.  Try again.")
        if not unique_abbreviation:
            print("We already have a department by that abbreviaiton.  Try again.")
...
def select_department(db):
    ...
    while not found:
        abbreviation = input("Department abbreviation--> ")
        department_count: int = collection.count_documents({"abbreviation": abbreviation})
        found = department_count == 1
        if not found:
...
if __name__ == '__main__':
    ...
    username = "Cluster22803"
    password = "wccFxLl0XlaUoYwX"
    project = "CECS 323 Summer 2024"
    hash_name = "xx9rtqy"
